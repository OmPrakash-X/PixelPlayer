...
class ImageCacheManager @Inject constructor(
    ...
    fun invalidateCoverArtCaches(vararg uriStrings: String?) {
        ...
        uriStrings.mapNotNull { it?.takeIf(String::isNotBlank) }.forEach { baseUri ->
            if (com.tencent.qqmusic.utils.LocalArtworkUri.isLocalArtworkUri(baseUri)) {
                com.tencent.qqmusic.utils.LocalArtworkUri.parseSongId(baseUri)?.let { songId ->
                    com.tencent.qqmusic.utils.AlbumArtUtils.clearCacheForSong(context, songId)
                }
            }
